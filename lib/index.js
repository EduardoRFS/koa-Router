...
Router.config = proto.config = function config (opts) {
  this.configStack.push({ test: opts.test, createHandler: opts.createHandler });
};
...
proto.use = function use (fn) {
  ...
  for (let fn of callbacks) {
    ...
    this.configStack.forEach(config => {
      if (config.test(fn))
        fn = config.createHandler(fn);
    });
